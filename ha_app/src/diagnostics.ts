/** Bounded support observations and opt-in logs. Never serialize upstream text or identities. */
import type { ChildProcess } from 'node:child_process';
export const diagnosticEvents = ['media_active_nvidia', 'media_active_software', 'media_hardware_failed', 'media_hardware_timeout', 'media_hardware_buffer_limit', 'media_software_fallback', 'fallback_startup_timeout', 'fallback_playback_timeout', 'fallback_connection_failed', 'fallback_signaling_error', 'fallback_playback_error', 'start', 'video_input', 'audio_input', 'jpeg_frame', 'media_output', 'media_reader', 'frame_ack', 'viewer_timeout', 'camera_timeout', 'session_end', 'stream_failure', 'no_viewers', 'h264', 'hevc', 'audio_supported', 'audio_absent', 'audio_late', 'jpeg_encoder_exit', 'media_encoder_exit', 'jpeg_encoder_error', 'media_encoder_error', 'jpeg_invalid_data', 'media_invalid_data', 'jpeg_decode_error', 'media_decode_error', 'jpeg_encoder_stderr', 'media_encoder_stderr', 'media_audio_error', 'audio_transport_error'] as const;
...
export class StreamDiagnostics {
  ...
  constructor(private readonly output: (line: string) => void = console.info, private readonly now = () => performance.now(), private readonly observe?: (serial: string, event: DiagnosticEvent) => void) {}
  begin(serial: string): void {
    if ((!this.enabled && !this.observe) || this.sessions.size >= 8 && !this.sessions.has(serial)) return;
  ...
  mark(serial: string, event: DiagnosticEvent): void {
    ...
    if (session && diagnosticEvents.includes(event)) { try { this.observe?.(serial, event); } catch { /* Observation cannot affect playback. */ } }
    if (!this.enabled || !session || !diagnosticEvents.includes(event) || session.seen.has(event)) return;
  ...
  encoder(serial: string, kind: 'jpeg' | 'media', process: ChildProcess): void {
    if (!this.enabled && !this.observe) { process.stderr?.resume(); return; }
    ...
    process.stderr?.on('data', (chunk: Buffer) => {
      ...
      if (kind === 'media' && /\[aac[^\]]* @/i.test(tail)) mark('media_audio_error');

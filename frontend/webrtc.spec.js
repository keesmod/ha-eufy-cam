...
for (const {ending,profile} of cases) test(`real WebRTC ${profile} stops after ${ending}`, async ({ page }, testInfo) => {
  ...
  const media = new MediaRelay(()=>hub.end('CAM123','Media failed'), undefined, serial => fake.emit('audio-ready',serial));
  ...
  try {
    ...
    await page.exposeFunction('backendClose',()=>{audioSignaling?.close();signaling?.close();cameraWs?.close();});

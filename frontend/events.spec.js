...
test('105 events, camera filter, bounded previews, marked calendar and adjacent real playback',async({page})=>{
 ...
 await page.getByRole('button',{name:'Close',exact:true}).click();expect(await page.locator('video').getAttribute('src')).toBeNull();expect((await page.evaluate(()=>urls)).some(u=>u.includes('/live')||u.includes('/snapshot'))).toBe(false);
});

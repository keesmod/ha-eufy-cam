...
export class EufyViewerCard extends HTMLElement {
    ...
    constructor() {
      ...
      <dialog aria-labelledby="live-title"><div class="bar"><span id="live-title"></span><button class="close" type="button"></button></div><img class="live" alt=""></dialog>`;
        ...
        this.shadowRoot.querySelector(".close").addEventListener("click", () => this._stop());
    ...
    _render() {
        ...
        this.shadowRoot.querySelector(".close").textContent = text.close;
        ...
        this.shadowRoot.querySelector(".age").textContent = date && !Number.isNaN(date.valueOf()) ? `${text.received}: ${date.toLocaleString(this._hass.language)} · ${text.unknown}` : text.noSnapshot;

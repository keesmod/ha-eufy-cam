...
export class EufyViewerCard extends HTMLElement {
    ...
    constructor() {
      ...
      <style>
        ...
        .meta{padding:16px}.name{font-weight:600;font-size:16px;line-height:24px}.age:empty,.status:empty{display:none}.age,.status{font-size:13px;color:var(--secondary-text-color);margin-top:5px}
      ...
      <ha-card>
        ...
        <div class="meta"><div class="name"></div><div class="age"></div><div class="status" role="status" aria-live="polite"></div><button class="close record-open" type="button"></button></div>
      </ha-card>
    ...
    _render() {
        ...
        this.shadowRoot.querySelector(".age").textContent = date && !Number.isNaN(date.valueOf()) ? `${text.received}: ${date.toLocaleString(this._hass.language)} · ${text.unknown}` : "";
    ...
    async _loadRecordings() {
        ...
        try {
            ...
            if (generation !== this._recordGeneration || !this._recordDialog.open)
                return;
            if (!Array.isArray(data.recordings) || data.recordings.length > 1000)

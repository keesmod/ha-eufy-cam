const EUFY_VIEWER_CARD_VERSION = "0.8.23";
"use strict";
...
export class EufyViewerCard extends HTMLElement {
    ...
    _autostart() {
        if (!this._autostartPending)
            return;
        if (!this._config?.live_autostart || !this._inline || this._autostartBlocked || this._open || !this.isConnected || !this._visible || document.visibilityState !== "visible") {
            this._autostartPending = false;
            return;
        }
        // A camera that is unavailable at the trigger keeps it until a later state update. Open recordings keep it until they close.
        if (!this._hass || this._preview.disabled || this._recordDialog.open)
            return;
        this._autostartPending = false;
        void this._start();
    }

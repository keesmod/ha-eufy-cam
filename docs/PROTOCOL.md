...
- Native playback uses authenticated `POST /api/eufy_viewer/recordings/{entity_id}/{id}/playback`. It prepares one MP4 and returns a playback path plus a five-minute HA-signed URL. The signature is tied to the requesting refresh token and exact path; it is not a long-lived access token.

...
These commands check out release 0.6.5 and build its Docker image locally:

```sh
git clone --branch v0.6.5 --depth 1 https://github.com/keesmod/ha-eufy-cam.git
cd ha-eufy-cam
docker build -t eufy-viewer-bridge:0.6.5 ./bridge
```
...
These steps update an older Docker installation to 0.6.5. Run them from your existing checkout and keep the original `bridge.env`. If you used a different container name, volume name, image tag or port mapping, use those values instead of the examples below.

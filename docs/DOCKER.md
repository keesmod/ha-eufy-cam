...
These commands check out release 0.4.0 and build its Docker image locally:

```sh
git clone --branch v0.4.0 --depth 1 https://github.com/keesmod/ha-eufy-cam.git
cd ha-eufy-cam
...
These steps update an older Docker installation to 0.4.0. Run them from your existing checkout and keep the original `bridge.env`. If you used a different container name, volume name, image tag or port mapping, use those values instead of the examples below.
...
6. [Update the HACS integration and card resource](../README.md#upgrade-from-01x-02x-or-03x) to the matching release.

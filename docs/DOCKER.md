...
Optional settings: `-e EUFY_LIVE_MAX_BITRATE=2500k` caps the live encoder (default `4M`), `-e EUFY_LIVE_MAX_STREAMS_PER_STATION=2` lets that many cameras on one HomeBase stream live at the same time (a whole number from 1 to 4, default 1), `-e EUFY_LIVE_MAX_SECONDS_MAINS=1800` raises the live session cap only for cameras without a battery value (120 to 3600 seconds, default 120, verified with one mains powered camera for a full 1800-second session), and `-e EUFY_DIAGNOSTICS=true` adds bounded live diagnostics to the log. See [playback and recovery](USAGE.md#honest-snapshot-and-streaming-limits).

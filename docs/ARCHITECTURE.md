...
- The card disables automatic resubscription. It releases late subscription results if the original live view has closed and cancels on pagehide, visibility loss, intersection loss, removal, Escape, close or disconnection. The inline live mode reuses the same elements and rules inside the card instead of the modal dialog, and one card still owns one lease. The optional `live_autostart` of an inline card is explicit configuration: a view opening, a return to visibility or a scroll into view starts at most one session per card, and a session that ended, was refused, paused or stopped never restarts by itself.

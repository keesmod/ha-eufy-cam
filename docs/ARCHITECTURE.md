...
- The card disables automatic resubscription. It releases late subscription results if the original live view has closed and cancels on pagehide, visibility loss, removal, Escape, close or disconnection, and on intersection loss unless the card is an inline card with `live_autostart`. The inline live mode reuses the same elements and rules inside the card instead of the modal dialog, and one card still owns one lease. The optional `live_autostart` of an inline card is explicit configuration: the card coming into view after it is attached or a return to page visibility starts at most one session per card, the session and its acknowledgement loop continue while the card is scrolled out of view, and a session that ended, was refused, paused or stopped never restarts by itself, scrolling included.
...
- The bridge owns reference counting, start/stop, the 10-second lease and the hard session cap independently of Home Assistant. The cap is 120 seconds, raised only for cameras without a battery value when `live_max_seconds_mains` is set, and the same value is passed to the client, which sends the STOP at the bound. Home Assistant's relay bounds itself by the same cap for the camera plus five seconds, so a bridge socket that neither sends nor closes still ends, and HA never ends a session the bridge still permits. A slow viewer cannot create an unbounded queue. Video buffers and parser output have hard limits.
...
A decoded-frame acknowledgement is a cooperative client signal, not proof a human is looking at the screen. No remote controller can guarantee physical stop through network partition or total power/process failure. The local watchdog bounds an absent viewer while the bridge is alive; firmware/P2P teardown behavior needs hardware acceptance testing. The session cap in the SDK timer is process-local, not a camera-firmware guarantee. A HomeBase 3 kept a ten-minute stream running on the library's bench, so the cap protects battery cameras and viewer discipline, not the station.
